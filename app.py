...
def get_videos():
    ...
    # Get ALL completed videos from DB
    from sqlalchemy import func
    ...
    videos = query.all()
    # Keep all video database records
    valid_videos = videos
...
@app.route('/api/scrape/<int:scrape_id>/videos')
@login_required
def get_scrape_videos(scrape_id):
    scrape = Scrape.query.get_or_404(scrape_id)
    # Allow any logged-in user to see videos of the scrape (for local downloading)
    return jsonify([{
        'id': v.id,
        'platform': v.platform,
        'url': v.url,
...
@app.route('/api/video/<int:video_id>/download_request', methods=['POST'])
@login_required
def download_request(video_id):
    video = Video.query.get_or_404(video_id)
    scrape = Scrape.query.get(video.scrape_id)
    try:
        from scraper import download_video_task
        # Run download_video_task synchronously
        download_video_task(video.id, video.url, scrape.id, app, db, Video, Scrape, CACHE_FOLDER)
...
@app.route('/api/video/<int:video_id>/cleanup_server_file', methods=['POST'])
@login_required
